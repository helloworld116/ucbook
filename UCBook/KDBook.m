...
#import "KDBook.h"
#import "SqlUtils.h"
#define kBookEncoding -2147482062
...
- (unsigned long long)fixOffserWith:(NSFileHandle *)handle{
	...
	if (oData) {
		NSString *jStr = [[NSString alloc]initWithData:oData encoding:kBookEncoding];
		if (jStr) {
			char *oCh = (char *)[oData bytes];
			while  ((*oCh >= 65 && *oCh <= 90) || (*oCh >= 97 && *oCh <= 122)) {								
				[handle seekToFileOffset:--offset];									
				NSData *jData = [handle readDataOfLength:1];
				NSString *kStr = [[NSString alloc]initWithData:jData encoding:kBookEncoding];
...
//        NSString* bookName = singleBook.bookFile;
		NSFileHandle *handle = [self handleWithFile:bookFilePath];
		NSData *data = [handle readDataOfLength:[[pageIndexArray objectAtIndex:0] unsignedLongLongValue]];
		NSString *string = [[[NSString alloc] initWithData:data encoding:kBookEncoding] autorelease];
		if (string) {
...
- (unsigned long long)indexOfPage:(NSFileHandle *)handle textFont:(UIFont *)font{
	...
	do{		
		for (int j=0; j<3; j++) {
			...
			if (data) {
//                [NSString stringWithContentsOfFile:bookFilePath encoding:-2147482062 error:nil]
				NSString *iStr = [[NSString alloc] initWithData:data encoding:kBookEncoding];
				if (iStr ) {					
...
//    NSString* bookName = singleBook.bookFile;
	...
	NSString *labelText = [[[NSString alloc] initWithData:data encoding:kBookEncoding] autorelease];
	if (labelText == nil) {
		return nil;
	}
...
//    RJSingleBook* singleBook = [[RJBookData sharedRJBookData].books objectAtIndex:bookIndex];
//    NSString* bookName = singleBook.bookFile;
	bookSize = [self fileLengthWithFile:bookFilePath];
	NSFileHandle *handle = [self handleWithFile:bookFilePath];
...
- (id)initWithBook:(NSInteger)newBookIndex chapterId:(NSInteger)newChapterId{
	self = [self init];
	if (self) {
        ...
        bookFilePath=[[NSHomeDirectory() stringByAppendingPathComponent:filePath] retain]; 
